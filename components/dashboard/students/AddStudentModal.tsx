            ...
            >
              ...
              <form 
                onSubmit={async (e) => {
                  e.preventDefault();
                  if (!validateForm()) {
                    toast.error(t('please_fix_errors_form'));
                    return;
                  }
                  ...
                  try {
                    if (isEditing && editingStudent) {
                      ...
                      if (!result.success) {
                        if (result.errors) {
                          ...
                          toast.error(t('validation_error'), { description: firstError as string });
                        } else {
                      ...
                      if (!finalStudentId && activeAcademicYear) {
                        try {
                          ...
                          toast.error(t('failed_to_generate_student_id'));
                          setIsSubmitting(false);
                          return;
                        }
                      }
                      ...
                      if (!finalStudentId) {
                        toast.error(t('student_id_missing'));
                        setIsSubmitting(false);
                        return;
                      }
                      ...
                      if (!result.success) {
                        if (result.errors) {
                          ...
                          toast.error(isMissingColumn ? t('database_schema_error') : t('registration_failed'), { 
                      ...
                      toast.success(t('student_registered_success'), {
                        description: t('student_added_to_grade').replace('{name}', formData.name).replace('{grade}', formData.grade)
                      });
                    }
                    ...
                    toast.error(t('failed_to_register_student'));
                  } finally {
                    setIsSubmitting(false);
                  }
                }} 
              ...
              >
                ...
                <div className="pt-4 border-t border-border">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                    ...
                    <div className="space-y-2">
                      <label className="text-sm font-bold text-foreground/80 mb-1 block">{t('parent_email_address')}</label>
                      <input 
                        required 
                        type="email" 
                ...
                <div className="pt-4 border-t border-border">
                  <h3 className="font-bold text-foreground mb-4">{t('registration_finance_details')}</h3>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                    <div className="space-y-2 text-foreground">
                      <label className="text-sm font-bold text-foreground/80 mb-1 block">{t('date_of_joining')}</label>
                      <input 
                        type="date" 
                        ...
                        className={`w-full px-4 py-3 rounded-xl border bg-muted/50 text-foreground focus:bg-background focus:ring-4 outline-none transition-all font-medium border-border focus:border-primary focus:ring-primary/20`} 
                      />
                      <p className="text-[10px] text-muted-foreground mt-1">{t('joining_date_desc')}</p>
                    </div>
                    <div className="space-y-2 text-foreground">
                      <label className="text-sm font-bold text-foreground/80 mb-1 block">{t('sibling_discount_scholarship')}</label>
                      <input 
                        type="number" 
                        min="0"
                        max="100"
                  ...
                  <div className="space-y-4">
                    <div className="space-y-2">
                       <label className="text-xs font-bold text-muted-foreground uppercase tracking-wider">{t('select_grade_fee_structure')}</label>
                       <select 
                        ...
                        >
                          <option value="">{t('select_fee_structure_option')}</option>
                          {feeItems.map((item) => (
                       ...
                       <label className="flex items-center gap-2 mt-3 cursor-pointer">
                         ...
                         />
                         <span className="text-sm font-bold text-foreground">{t('custom_fees_amount')}</span>
                       </label>

                       {formData.isCustomFee && (
                         <div className="mt-3">
                           <label className="block text-[10px] font-bold text-muted-foreground uppercase mb-1">{t('manual_amount')}</label>
                           <input 
                              type="number" 
                              placeholder="0.00"
                    ...
                    <div className="space-y-2 mt-4 pt-4 border-t border-border">
                        <label className="text-xs font-bold text-muted-foreground uppercase tracking-wider">{t('select_payment_structure')}</label>
                        <select 
                        ...
                        >
                          <option value="1 Term">{t('term_payment_installment').replace('{count}', '1')}</option>
                          <option value="2 Terms">{t('term_payment_installment').replace('{count}', '2')}</option>
                          <option value="3 Terms">{t('term_payment_installment').replace('{count}', '3')}</option>
                          <option value="4 Terms">{t('term_payment_installment').replace('{count}', '4')}</option>
                          <option value="5 Terms">{t('term_payment_installment').replace('{count}', '5')}</option>
                          <option value="6 Terms">{t('term_payment_installment').replace('{count}', '6')}</option>
                        </select>
                      </div>
                  </div>
                </div>
