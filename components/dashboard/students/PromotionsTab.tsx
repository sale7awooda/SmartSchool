...
export function PromotionsTab({ activeAcademicYear, mutateStudents, t }: any) {
  ...
  const getExamStatus = (studentId: string) => {
    ...
    if (studentResults.length === 0) return 'not_applicable';
    
    if (studentResults.length < 3) return 'not_yet_promoted';
  ...
  const handlePromote = async () => {
    if (selectedStudents.size === 0) {
      toast.error(t('please_select_students_promote') || 'Please select students to promote');
      return;
    }
    ...
    try {
      ...
      toast.success(t('successfully_promoted').replace('{count}', selectedStudents.size.toString()));
      mutateStudents(); 
      ...
      console.error(error);
      toast.error(t('failed_to_promote'));
    } finally {
      ...
      <div className="bg-card dark:bg-slate-900 p-6 rounded-[1.5rem] border border-border dark:border-slate-800 shadow-sm shrink-0">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          <div>
            ...
            <p className="text-muted-foreground mt-1 text-sm">{t('student_promotions_desc')}</p>
          </div>
          <button
          ...
          >
            ...
            {t('promote_selected').replace('{count}', selectedStudents.size.toString())}
          </button>
        </div>
            ...
            <table className="w-full text-sm text-left rtl:text-right">
              <thead className="text-xs text-muted-foreground uppercase bg-muted/50 border-b border-border">
                <tr>
                  ...
                  <th className="px-6 py-4 font-bold">{t('current_grade')}</th>
                  <th className="px-6 py-4 font-bold">{t('exam_status')}</th>
                  <th className="px-6 py-4 font-bold">{t('promote_to')}</th>
                </tr>
              </thead>
              <tbody>
                {isLoading ? (
                  ...
                  <tr><td colSpan={5} className="p-8 text-center text-muted-foreground">{t('no_students_found')}</td></tr>
                ) : (
                  filteredStudents.map((student: any) => {
                    const statusKey = getExamStatus(student.id);
                    const canPromote = statusKey !== 'not_applicable';
                      ...
                      <tr key={student.id} className="border-b border-border last:border-0 hover:bg-muted/30">
                        ...
                        <td className="px-6 py-4">
                          <span className={`px-2.5 py-1 rounded-md text-xs font-bold ${
                            statusKey === 'passed' ? 'bg-emerald-500/10 text-emerald-500' : 
                            statusKey === 'demoted' ? 'bg-amber-500/10 text-amber-500' :
                            statusKey === 'not_applicable' ? 'bg-muted text-muted-foreground' :
                        ...
                        <td className="px-6 py-4">
                          <select
                            value={nextGrades[student.id] || (statusKey === 'demoted' ? student.grade : suggestNextGrade(student.grade))}
