...
export function DirectoryTab({ onSelectStaff, onAddEmployee }: { onSelectStaff: (staff: any) => void, onAddEmployee: () => void }) {
  ...
  const [selectedIdCardStaff, setSelectedIdCardStaff] = useState<any>(null);
  const limit = 10;
                ...
                <tr key={staff.id} className="hover:bg-accent/50 transition-colors">
                  ...
                  <td className="p-4 text-right">
                    <div className="flex items-center justify-end gap-2">
                      <button 
                        onClick={() => setSelectedIdCardStaff(staff)}
                        className="px-2.5 py-1.5 bg-emerald-500/10 text-[11px] text-emerald-600 dark:text-emerald-400 rounded-lg font-bold hover:bg-emerald-500/20 transition-colors cursor-pointer"
                      >
      ...
      <AnimatePresence>
        {selectedIdCardStaff && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-background/80 backdrop-blur-sm">
            ...
            >
              <button 
                onClick={() => setSelectedIdCardStaff(null)}
              ...
              <h3 className="text-lg font-bold mb-4 text-center">Staff ID Card Export</h3>

              {/* ID Card Wrapper */}
              <div className="p-4 bg-muted/30 border border-border rounded-3xl mb-6">
                <div id="staff-id-card-print" className="relative w-72 h-[420px] bg-gradient-to-br from-card to-muted border border-border rounded-[1.5rem] overflow-hidden shadow-xl flex flex-col items-center p-5 text-foreground">
                  {/* Header banner */}
                  <div className="absolute top-0 inset-x-0 h-20 bg-emerald-600 flex flex-col justify-center items-center text-white p-3">
                    <div className="flex items-center gap-1">
                      <Shield size={18} className="text-white shrink-0" />
                      <span className="font-extrabold text-xs tracking-tight uppercase">Smart School HR</span>
                    </div>
                    <span className="text-[8px] font-bold opacity-80 uppercase tracking-widest mt-0.5">Staff Identity Card</span>
                  </div>

                  {/* Avatar section */}
                  <div className="mt-12 relative">
                    <div className="w-24 h-24 rounded-full border-4 border-card bg-emerald-500/30 flex items-center justify-center overflow-hidden shadow-md">
                      <span className="text-2xl font-extrabold text-emerald-700 dark:text-emerald-300">
                        {selectedIdCardStaff.name?.charAt(0).toUpperCase()}
                      </span>
                    </div>
                    <div className="absolute -bottom-1 -right-0.5 w-6 h-6 rounded-full bg-emerald-500 border border-card flex items-center justify-center shadow-sm">
                  ...
                  <div className="mt-3 text-center space-y-1 w-full">
                    <h3 className="text-md font-black text-foreground tracking-tight truncate px-2">{selectedIdCardStaff.name}</h3>
                    <span className="px-2 py-0.5 bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 border border-emerald-500/20 rounded-full text-[10px] font-bold inline-block capitalize">
                      {selectedIdCardStaff.role || 'Staff member'}
                    </span>
                  </div>

                  <div className="mt-4 w-full space-y-2 px-4 border-t border-b border-border/80 py-3 text-left">
                    <div className="flex justify-between text-[10px]">
                      <span className="text-muted-foreground font-bold uppercase">STAFF ID</span>
                      <span className="text-foreground font-mono font-bold truncate max-w-[120px] text-right">{selectedIdCardStaff.id?.slice(0, 8).toUpperCase() || 'N/A'}</span>
                    </div>
                    <div className="flex justify-between text-[10px]">
                      <span className="text-muted-foreground font-bold uppercase">EMAIL</span>
                      <span className="text-foreground font-mono font-bold truncate max-w-[120px] text-right">{selectedIdCardStaff.email || 'N/A'}</span>
                    </div>
                    <div className="flex justify-between text-[10px]">
                      <span className="text-muted-foreground font-bold uppercase">STATUS</span>
                      <span className="text-emerald-600 font-mono font-bold">{selectedIdCardStaff.status || 'Active'}</span>
                    </div>
                  </div>
                  ...
                  <div className="mt-auto w-full flex flex-col items-center gap-1.5">
                    <div className="h-6 w-44 bg-foreground/10 border-l border-r border-foreground/30 flex items-center justify-center font-mono text-[8px] tracking-[5px] text-foreground select-none">
                      *{selectedIdCardStaff.id?.slice(0, 6).toUpperCase() || 'STAFF'}*
                    </div>
                    <span className="text-[7px] text-center text-muted-foreground font-semibold uppercase tracking-wider">Smart School HR Division</span>
                  </div>
                </div>
              </div>
              ...
              <div className="flex gap-3 w-full">
                ...
                <button 
                  onClick={() => {
                    const printContent = document.getElementById('staff-id-card-print');
                    ...
                    printWindow.document.write(`
                      <html>
                        <head>
                          <title>Staff ID Card - \${selectedIdCardStaff.name}</title>
