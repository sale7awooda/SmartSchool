export async function fetchRoute(coordinates: {lat: number, lng: number}[]) {
  ...
  if (!mapboxToken) {
    // Fallback to purely free/open-source OSRM
    ...
    try {
      ...
      if (data.code === 'Ok' && data.routes.length > 0) {
        ...
        const routeCoordinates = route.geometry.coordinates.map((c: [number, number]) => [c[1], c[0]]); // [lng, lat] to [lat, lng]
  ...
  // Use Mapbox if token is present
