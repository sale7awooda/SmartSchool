...
async function mapUsersToRecords(records: any[], userIdField: string) {
  ...
  const { data: users } = await supabase.from('users').select('id, name, role, phone').in('id', userIds);
  const userMap = (users || []).reduce((acc: any, u: any) => ({ ...acc, [u.id]: u }), {});
  return records.map(r => {
    const u = userMap[r[userIdField]];
    const name = u?.name || r.staff?.name || r[userIdField];
    const role = u?.role || '';
    const phone = u?.phone || '';
    return { ...r, staffName: name, staff: name, staffRole: role, staffPhone: phone };
  });
}
