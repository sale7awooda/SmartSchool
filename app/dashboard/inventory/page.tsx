...
import { getPaginatedInventory, createInventoryItem, updateInventoryItem, deleteInventoryItem } from '@/lib/supabase-db';
...
export default function InventoryPage() {
  ...
  const handleMaintenance = async (id: string) => {
    const nextDate = prompt(t('enter_next_maintenance_date'), new Date().toISOString().split('T')[0]);
    if (!nextDate) return;
    try {
                      ...
                      <td className="p-4">
                        <div className="flex items-center gap-3">
                          <div className="w-10 h-10 rounded-xl bg-muted flex items-center justify-center text-muted-foreground">
                            ...
                            {!['Laptop', 'Projector', 'Sports'].includes(item.category) && <Package size={20} />}
                          </div>
                          <div>
                            <p className="font-bold text-foreground text-sm">{item.name}</p>
                            <p className="text-xs text-muted-foreground font-medium">Qty: {item.quantity || 1} • {item.category}</p>
                          </div>
                        </div>
                      </td>
                      <td className="p-4">
                        <p className="text-sm font-bold text-foreground">{item.assigned_to || 'Unassigned'}</p>
                      </td>
                      <td className="p-4">
                        <span className={`px-3 py-1 rounded-lg text-xs font-bold ${
                          item.status === 'Available' ? 'bg-emerald-500/20 text-emerald-500' : 
                  ...
                  <form onSubmit={handleAddAsset} className="p-6 sm:p-8 space-y-5 overflow-y-auto custom-scrollbar">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      ...
                      <div>
                        ...
                        <input required name="quantity" type="number" min="1" defaultValue="1" className="w-full px-4 py-3.5 rounded-xl border border-border bg-muted/50 focus:bg-background focus:border-primary focus:ring-4 focus:ring-primary/20 outline-none transition-all font-medium text-foreground" />
                      </div>
                    </div>

                    <div>
                      <label className="block text-sm font-bold text-foreground mb-2">{t('assigned_to')}</label>
                      <input name="assigned_to" type="text" placeholder="e.g., Room 101" className="w-full px-4 py-3.5 rounded-xl border border-border bg-muted/50 focus:bg-background focus:border-primary focus:ring-4 focus:ring-primary/20 outline-none transition-all font-medium text-foreground placeholder:text-muted-foreground" />
                    </div>

                    <div>
                      <label className="block text-sm font-bold text-foreground mb-2">{t('next_maintenance_date')}</label>
                      <input name="next_maintenance_date" type="date" className="w-full px-4 py-3.5 rounded-xl border border-border bg-muted/50 focus:bg-background focus:border-primary focus:ring-4 focus:ring-primary/20 outline-none transition-all font-medium text-foreground" />
                    </div>
