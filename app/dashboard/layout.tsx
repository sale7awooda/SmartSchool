...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  if (isLoading) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-background text-foreground gap-4">
        ...
        <p className="text-muted-foreground font-medium animate-pulse">{t('loading_dashboard')}</p>
      </div>
    );
  }
  ...
  const getNavItems = () => {
    const items = [
      ...
      { 
        name: (user.role === 'student' || user.role === 'parent') ? t('profile') : t('students'), 
        href: '/dashboard/students', 
        icon: (user.role === 'student' || user.role === 'parent') ? User : Users, 
        show: (user.role === 'student' || user.role === 'parent') ? true : can('view', 'students') 
      },
      { name: t('exams'), href: '/dashboard/assessments', icon: FileText, show: can('view', 'assessments') },
      { name: t('report_cards'), href: '/dashboard/report-cards', icon: GraduationCap, show: can('view', 'assessments') || can('view', 'academics') },
      ...
      { name: t('audit_logs'), href: '/dashboard/audit-logs', icon: Shield, show: user.role === 'admin' },
    ...
    <div className="min-h-screen w-full bg-muted/30 dark:bg-background flex flex-col md:flex-row font-sans transition-colors">
      ...
      >
        <div className="p-4 border-b border-border/50 flex items-center justify-center h-24 relative">
          <div className={`flex items-center justify-start gap-4 overflow-hidden transition-all duration-300 ${isCollapsed ? 'w-0 opacity-0' : 'w-full opacity-100'}`}>
            <Logo withText size={42} className="w-full justify-start pl-2" />
          </div>
