...
export default function AdminScheduleView() {
  ...
  const { data: schedulesData, isLoading: isSchedulesLoading, mutate: mutateSchedules } = useSWR(
    ['schedules-admin', activeAcademicYear?.name || 'all'],
  ...
  const grades = useMemo(() => {
    const rawGrades = (classesData?.map(c => c.name) || [
      'PRE-K', 'KG1', 'KG2',
      'Grade 1', 'Grade 2', 'Grade 3', 'Grade 4', 'Grade 5',
      'Grade 6', 'Grade 7', 'Grade 8', 'Grade 9', 'Grade 10'
    ]).filter(name => !['PRE-K', 'KG1', 'KG2'].includes(name.toUpperCase().trim()));
  ...
  useEffect(() => {
    ...
    if (teachers.length > 0 && !selectedTeacher && user?.role !== 'teacher') {
