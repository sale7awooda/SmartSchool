...
const MOCK_STAFF = [
  ...
  { id: '4', name: 'Edna Krabappel', role: 'teacher', department: 'Elementary Education', email: 'teacher@smartschool.com', phone: '555-0102', status: 'Active', designation: 'Grade 4 Teacher', joinDate: '1998-08-20', qualifications: ['B.Ed. Elementary Education', 'M.A. Curriculum Development'], subjects: ['Mathematics', 'English', 'Social Studies'] },
  { id: '5', name: 'Willie MacDougal', role: 'staff', department: 'Facilities', email: 'staff@smartschool.com', phone: '555-0103', status: 'Active', designation: 'Head Groundskeeper', joinDate: '1995-05-01', qualifications: ['Certified Landscaper', 'Boiler Maintenance Specialist'], subjects: [] },
];
...
export default function HRPage() {
  ...
  const handleAddEmployee = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    // Map specific fields for extra_info
    const extraInfoObj = {
      account_number: formData.get('account_number'),
      other_info: formData.get('other_info')
    };
    formData.set('extra_info', JSON.stringify(extraInfoObj));

    // Map specific fields to department based on role
    if (newEmployeeRole === 'teacher') {
      formData.set('department', (formData.get('designation') as string) || 'Teaching');
    } else if (newEmployeeRole === 'staff') {
      formData.set('department', (formData.get('designation') as string) || 'General Staff');
              ...
              <form onSubmit={handleAddEmployee} className="p-6 sm:p-8 space-y-5 overflow-y-auto custom-scrollbar">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    ...
                    <input name="phone" required type="tel" placeholder="+1 234 567 8900" className="w-full px-4 py-3.5 rounded-xl border border-border bg-muted/50 focus:bg-background focus:border-primary focus:ring-4 focus:ring-primary/20 outline-none transition-all font-medium text-foreground placeholder:text-muted-foreground" />
                  </div>
                  <div>
                    <label className="block text-sm font-bold text-foreground mb-2">Date of Birth</label>
                    <input name="dob" required type="date" className="w-full px-4 py-3.5 rounded-xl border border-border bg-muted/50 focus:bg-background focus:border-primary focus:ring-4 focus:ring-primary/20 outline-none transition-all font-medium text-foreground" />
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    ...
                    <select name="gender" required className="w-full px-4 py-3.5 rounded-xl border border-border bg-muted/50 focus:bg-background focus:border-primary focus:ring-4 focus:ring-primary/20 outline-none transition-all font-medium text-foreground">
                  ...
                  <div>
                    ...
                    <input name="date_of_join" required type="date" className="w-full px-4 py-3.5 rounded-xl border border-border bg-muted/50 focus:bg-background focus:border-primary focus:ring-4 focus:ring-primary/20 outline-none transition-all font-medium text-foreground" />
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    ...
                    <input name="salary" required type="number" step="0.01" placeholder="5000" className="w-full px-4 py-3.5 rounded-xl border border-border bg-muted/50 focus:bg-background focus:border-primary focus:ring-4 focus:ring-primary/20 outline-none transition-all font-medium text-foreground placeholder:text-muted-foreground" />
                  </div>
                  <div>
                    <label className="block text-sm font-bold text-foreground mb-2">Bank Account Number</label>
                    <input name="account_number" type="text" placeholder="Account Number" className="w-full px-4 py-3.5 rounded-xl border border-border bg-muted/50 focus:bg-background focus:border-primary focus:ring-4 focus:ring-primary/20 outline-none transition-all font-medium text-foreground placeholder:text-muted-foreground" />
                  </div>
                </div>

                <div>
                   <label className="block text-sm font-bold text-foreground mb-2">Certificate / Qualifications</label>
                   <input name="education" type="text" placeholder="e.g. B.Sc. Education, Teaching License" className="w-full px-4 py-3.5 rounded-xl border border-border bg-muted/50 focus:bg-background focus:border-primary focus:ring-4 focus:ring-primary/20 outline-none transition-all font-medium text-foreground placeholder:text-muted-foreground" />
                </div>

                <div>
                   <label className="block text-sm font-bold text-foreground mb-2">Address</label>
                   <textarea name="address" rows={2} placeholder="Full address" className="w-full px-4 py-3.5 rounded-xl border border-border bg-muted/50 focus:bg-background focus:border-primary focus:ring-4 focus:ring-primary/20 outline-none transition-all font-medium text-foreground placeholder:text-muted-foreground resize-none"></textarea>
                </div>

                <div>
                   <label className="block text-sm font-bold text-foreground mb-2">Other Related Info</label>
                   <textarea name="other_info" rows={2} placeholder="Any other notes..." className="w-full px-4 py-3.5 rounded-xl border border-border bg-muted/50 focus:bg-background focus:border-primary focus:ring-4 focus:ring-primary/20 outline-none transition-all font-medium text-foreground placeholder:text-muted-foreground resize-none"></textarea>
                </div>
