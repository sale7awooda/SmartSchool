                      ...
                      <td className="p-4">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            <p className="text-xs text-muted-foreground font-medium">Badge: {visitor.id.split('-')[0]}</p>
                          </div>
                        </div>
                      </td>
                      <td className="p-4">
                        <p className="text-sm font-bold text-foreground">{visitor.purpose}</p>
                        <p className="text-xs text-muted-foreground font-medium">Host: {visitor.host || 'General'}</p>
                      </td>
                      <td className="p-4">
                        <p className="text-sm font-bold text-foreground">In: {visitor.check_in ? new Date(visitor.check_in).toLocaleTimeString() : 'N/A'}</p>
                        {visitor.check_out && <p className="text-xs text-muted-foreground font-medium">Out: {new Date(visitor.check_out).toLocaleTimeString()}</p>}
                      </td>
                      <td className="p-4">
                        <span className={`px-3 py-1 rounded-lg text-xs font-bold ${visitor.status === 'Active' ? 'bg-emerald-500/20 text-emerald-500' : 'bg-muted text-muted-foreground'}`}>
                          {visitor.status}
                        </span>
                      </td>
                      <td className="p-4 text-right">
