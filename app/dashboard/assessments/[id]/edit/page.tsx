...
export default function EditAssessmentPage() {
  ...
  useEffect(() => {
    if (assessmentData && subjectsData && classesData && !defaultsSet) {
      setExamDetails({
        ...
        description: cleanDescription(assessmentData.description || ''),
      });
      if (assessmentData.questions && assessmentData.questions.length > 0) {
        const loadedQuestions = assessmentData.questions.map((q: any) => {
          ...
          let parsedCorrectArray: string[] = [];
          let parsedCorrectSingle = '0';
          if (q.correct_answer !== undefined && q.correct_answer !== null) {
             try { 
                const p = typeof q.correct_answer === 'string' ? JSON.parse(q.correct_answer) : q.correct_answer;
                if (Array.isArray(p)) parsedCorrectArray = p.map(String);
                else parsedCorrectSingle = String(p);
             } catch(e) { 
                parsedCorrectSingle = String(q.correct_answer); 
             }
          }
          if (q.correct_answers) {
             try { 
               const ca = typeof q.correct_answers === 'string' ? JSON.parse(q.correct_answers) : q.correct_answers; 
               if (Array.isArray(ca)) parsedCorrectArray = ca.map(String);
             } catch(e) {}
          }
  ...
  const { data: teacherSchedules } = useSWR(
    user && isTeacher ? `teacher_schedules_${user.id}` : null,
    async () => {
      if (!user) return [];
  ...
  const creatorId = assessmentData ? getCreatorId(assessmentData.description) : null;
  const isOwner = creatorId ? creatorId === user?.id : true; // Allow editing legacy assessments or assessments without tags

  const teacherSubjectNames = new Set(
    teacherSchedules?.map((s: any) => s.subject?.name?.toLowerCase().trim()) || []
  );
  const assessmentSubjectName = (assessmentData?.subject?.name || assessmentData?.subject || '').toLowerCase().trim();
  const isTeacherSubject = teacherSubjectNames.has(assessmentSubjectName);

  const canEdit = isOwner || isTeacherSubject;

  if (assessmentData && isTeacher && !canEdit && !isAdmin) {
    return (
      <div className="p-8 text-center bg-card border border-border rounded-[1.5rem] shadow-sm max-w-xl mx-auto my-12">
        <h2 className="text-xl font-bold text-destructive mb-2">Unauthorized Access</h2>
        <p className="text-muted-foreground text-sm font-medium mb-6">You are only permitted to edit assessments that you have created or that belong to your subject.</p>
        <Link href="/dashboard/assessments" className="px-4 py-2 bg-primary text-primary-foreground rounded-xl font-bold text-sm hover:bg-primary/90 transition-colors shadow-sm">
  ...
  const handleSave = async () => {
    ...
    setIsSaving(true);
    try {
      const existingCreatorId = getCreatorId(assessmentData?.description);
      const creatorIdToSave = existingCreatorId || user?.id;
      const cleanDesc = cleanDescription(examDetails.description);
      const descriptionWithCreator = cleanDesc + (cleanDesc ? '\n\n' : '') + `[CreatedBy: ${creatorIdToSave}]`;
