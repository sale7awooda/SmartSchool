...
export default function LoginPage() {
  ...
  const handleQuickLogin = async (role: string) => {
    ...
    setIsLoading(true);
    try {
      if (role === 'admin') await login('admin@smartschool.com', 'password123', true);
      else if (role === 'staff') await login('staff@smartschool.com', 'password123', true);
      else if (role === 'teacher') await login('teacher@smartschool.com', 'password123', true);
      else if (role === 'accountant') await login('accountant@smartschool.com', 'password123', true);
      else if (role === 'parent') await login('homer@smartschool.com', 'password123', true);
      else if (role === 'student') await login('student@smartschool.com', 'password123', true);
    } catch (err: any) {
      setError(err.message || 'Failed to login');
    } finally {
      setIsLoading(false);
        ...
        {isDevMode() && (
          ...
          >
            <div className="flex items-center gap-4 mb-4">
              ...
              <span className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">{t('mvp_quick_login') || 'Dev Quick Login'}</span>
              <div className="h-px bg-border flex-1" />
            </div>
